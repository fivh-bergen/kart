...
Expanding this list is quite straightforward, 
  just add a new tag to the \`tags\` array in [src/utils/tags.ts](https://github.com/fivh-bergen/kart/blob/main/src/utils/tags.ts) and run \`pnpm fetch-nodes\` to update the GeoJSON data and \`pnpm generate\` to update this table.`;
await fs.writeFile(
